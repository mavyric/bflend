...
# POST /v2/auth/w/funding/offer/cancel/all with symbol currency code (not f-symbol)
...
# POST /v2/auth/r/wallets returns balances; we can use /v2/auth/r/wallets
...
for w in resp:
wtype = w if len(w) > 0 else ""
currency = w if len(w) > 1 else ""
bal_av = safe_float(w if len(w) > 4 else 0.0)
...
# FRR proxy: use funding stats endpoint; last FRR is typically part of stats
# GET /v2/funding/stats/:SYMBOL/last
try:
data = _get(f"/v2/funding/stats/{SYMBOL}/last")
# Docs: returns [FRR, bid, ask] or similar; we take FRR-like field cautiously.
# If shape unknown, fall back to book mid estimate.
if isinstance(data, list) and len(data) >= 1:
frr = safe_float(data, 0.0)
if frr > 0:
return frr
except Exception as e:
...
# GET /v2/book/funding/fUST/R0?len=25
try:
book = _get(f"/v2/book/funding/{SYMBOL}/R0", params={"len": 25})
# Rows: [RATE, PERIOD, COUNT, AMOUNT] sign of amount distinguishes side
bids = []
asks = []
for row in book:
rate = safe_float(row)
amount = safe_float(row)
if amount > 0:
bids.append(rate)
elif amount < 0:
asks.append(rate)
...
# Bitfinex funding auto-renew flag: 1024 (0x400) historically; newer docs also mention 16384 (0x4000).
# Common community usage for funding auto-renew is 1024. We’ll use 1024 here.
...
for i, offset in enumerate(ladder):
    ...
    if amt < MIN_OFFER:
        break
    # FRRDELTA variable order: type=FRRDELTA, rate=offset
    try:
        resp = submit_offer(amt, offset, DURATION_D, "FRRDELTA", flags)
        print(f"FRR-Delta leg {i+1}: amount={amt:.2f} offset={offset:.6f} (daily), APY~{apy_to_str(daily_to_apy(base_frr+offset))} -> {resp}")
        remaining -= amt
        placed += 1
    except Exception as e:
        print(f"Error placing FRR-Delta leg {i+1}: {e}")
        # continue trying other legs/chunks
if low_env:
    print(f"Low APY environment detected (base APY {apy_to_str(base_apy)} < guard {min_apy_guard:.2f}%). Restricted to single FRR leg.")
return remaining
...
"""
if not ENABLE_MAKER_LEG or MAKER_CHUNKS <= 0:
return remaining
if remaining < MIN_OFFER:
return remaining
...
if best_bid:
    ...
    est_frr = fetch_frr_daily_rate()
    target_rate = max(est_frr, 0.0002)
...
for i in range(chunks):
    ...
    try:
        ...
        print(f"Maker leg {i+1}: amount={amt:.2f} rate={target_rate:.6f} (daily), APY~{apy_to_str(daily_to_apy(target_rate))} -> {resp}")
        remaining -= amt
    except Exception as e:
...
if remaining >= IDLE_WARN_THRESHOLD:
    print(f"Warning: {remaining:.2f} USDT still idle after this cycle. Consider increasing ladder size, maker chunks, or reducing MIN_APY_GUARD.")
